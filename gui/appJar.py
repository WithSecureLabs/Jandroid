...
#####################################
    def __init__(
        ...
        self.lib_path = os.path.join(gui.lib_path, "lib")
            ...
            try:
                if option == 'background':
                    ...
                    if self.ttkFlag:
                        ...
                        item.state([value])
                        if but is not None: but.state([btn_value])
                    else:
                        item.config(state=value)
                        if but is not None: but.config(state=btn_value)
...
#####################################
    ...
    def _buildTree(self, title, xmlDoc, row=None, column=0, colspan=0, rowspan=0):
        ...
        frame = ScrollPane(
            ...
            highlightthickness=0,
            takefocus=1)
        self._positionWidget(frame, row, column, colspan, rowspan, "NEW")
        ...
        class Separator(frameBase, object):
            ...
            def __init__(self, parent, orient="horizontal", *args, **options):
                ...
                if orient == "horizontal":
                    self.line.config(relief="groove", height=1, width=100, borderwidth=0)
                    self.line.pack(padx=0, pady=0, fill="x", expand=1)
                else:
                    self.line.config(relief="groove", height=100, width=1, borderwidth=1)
